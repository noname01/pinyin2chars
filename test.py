...
def get_log_cond_prob_bigram(w1, w2, smoothed_unigram_counts, unigram_p0, smoothed_bigram_counts, bigram_p0):
    return log(float(smoothed_bigram_counts.get(w1 + u" " + w2, bigram_p0))
               /(smoothed_unigram_counts.get(w1, unigram_p0) + len(smoothed_unigram_counts.keys())))
...
def convert_bigram_dp(pinyin_str, unigram_counts, bigram_counts, candidate_map):
    ...
    smoothed_bigram_counts, bigram_p0 = sgt.simpleGoodTuringProbs(bigram_counts)
    smoothed_unigram_counts, unigram_p0 = sgt.simpleGoodTuringProbs(unigram_counts)
    pinyins = re.split("\s+", pinyin_str)
    pinyins.insert(0, "<s>")
    ...
    for i in range(1, n):
        ...
        for cur_char in candidate_map[pinyins[i]]:
            ...
            for prev_char in prev_chars:
                ...
                sgtProbs, p0 = sgt.simpleGoodTuringProbs(bigram_counts)
                log_prob = get_log_cond_prob_bigram(prev_pair, cur_pair, smoothed_unigram_counts, unigram_p0, smoothed_bigram_counts, bigram_p0)
