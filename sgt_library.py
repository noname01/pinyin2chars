...
# pinyin_str: string of pinyin tokens, no start/end symbol
# returns a list of predicted characters
def convert_bigram_dp(pinyin_str, unigram_counts, bigram_counts, candidate_map, probDist):
    ...
    for i in range(1, n):
        ...
        for cur_char in candidate_map[pinyins[i]]:
            ...
            for prev_char in prev_chars:
                ...
                log_prob = probDist.logprob(prev_pair + u" " + cur_pair)
...
def get_accuracy(model_label, bitext_testing, unigram_counts, bigram_counts, candidate_map, probDist = None):
    total_chars = 0
    correct_chars = 0
    # count = 0
    for segment in bitext_testing:
        ...
        if model_label == "baseline":
            ...
            actual = convert_bigram_dp(pinyins, unigram_counts, bigram_counts, candidate_map, probDist)
        if actual == None:
...
if __name__ == "__main__":
    ...
    probDist = SimpleGoodTuringProbDist(freqDist)
    print probDist.prob(u"你 好")
    ...
    print("bigram")
    print(get_accuracy("bigram", bitext_testing, unigram_counts, bigram_counts, candidate_map, probDist))
